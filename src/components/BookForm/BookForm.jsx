...
const BookForm = ({ book, userProfile, handleAddBook, handleRemove }) => {
  ...
  useEffect(() => {
    ...
    const updatedList = collections.filter((collection) =>
      userProfile[collection].every((book) => book?.api_id !== formData.api_id)
    );
  ...
  return (
    <>
      <select
        ...
        className="border border-blue-500 my-1 group relative w-full flex justify-center py-1 border border-transparent text-sm font-medium rounded-md"
      >
      ...
      <button
        onClick={handleClick}
        className="group my-3 relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-500"
      >
